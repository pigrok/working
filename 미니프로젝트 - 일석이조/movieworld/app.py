...
#Like 하트 눌렀을 때 title로 조회 후 Like 카운트 증가하고 다시 누르면 Like 카운트 감소
@app.route("/movieworld_like", methods=["POST"])
def movie_like():
    title_receive = str(request.form['title'])
    like = db.movies.find_one({'title':title_receive})
    print(like['Like'])
    if like["Like"]  > 0 :
        db.movies.update_one({'title':title_receive},{"$set":{"Like": like['Like']-1}})
        msg = "싫어요!"
    else:
        db.movies.update_one({'title':title_receive},{"$set":{"Like": like['Like']+1}})
        msg = "좋아요!"
