    ...
    fetch('/movieworld').then((res) => res.json()).then((data) => {
        ...
        rows.forEach((a) => {
            ...
            let temp_html = `<div id="${genre}" class="box2 on">
                                <!-- 좋아요 눌렀을 떄 제목을 쉽게 가져오기 위함 -->
                                <i class="heart" data-title="${title}"> 
                                ...
                                <span class="delete_test" id="${m_id}" onclick="movieworld_delete(${m_id})">❌</span>
                                <ul class="list">
            ...
            togglingBtns.addEventListener('click', function (e) {
                togglingBtns.classList.toggle('active');
                // 성록 좋아요 업데이트 
                update_like(this.dataset.title)
            })
